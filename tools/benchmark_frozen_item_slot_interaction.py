...
def _run_preset(app, qml_path: Path, preset_id: str) -> dict:
    ...
    def quick_item(name: str) -> QQuickItem:
        ...
        usable = [item for item in matches if item.isVisible() and item.isEnabled() and item.width() > 0 and item.height() > 0]
        if usable:
            return usable[-1]
    ...
    def clipped_exclusions(item: QQuickItem) -> list[dict]:
        scene = item.mapToScene(QPointF(max(1.0, item.width()) / 2.0, max(1.0, item.height()) / 2.0))
        result: list[dict] = []
    ...
    def reveal_in_clipped_flickables(item: QQuickItem, margin: float = 32.0) -> list[dict]:
        ...
        while current is not None:
            ...
            is_flickable = "Flickable" in class_name(current) or (isinstance(content_x, (int, float)) and isinstance(content_y, (int, float)))
    ...
    def tracked_ids(snapshot: dict) -> dict[str, list[str]]:
        ...
        return {
            ...
            "PRICE": [str(price[k]) for k in ("currency_node", "integer_node", "decimal_node") if price.get(k)],
            "UNIT": [str(roles["unit"]["node_id"])],
        }

    def capture_roles(ids):
        return {role: [visual_rect(node_item(nid)) for nid in node_ids] for role, node_ids in ids.items()}

    def roles_changed(before, after) -> bool:
        for role in before:
            if not before[role] or len(before[role]) != len(after[role]):
                return False
            for left, right in zip(before[role], after[role]):
                if not any(abs(right[k] - left[k]) > 0.5 for k in ("x", "y", "width", "height")):
                    return False
        return True
    ...
    for index in range(1, EVENTS + 1):
        final_move = QPoint(move_start.x() + min(160, round(index * 0.8)), move_start.y() + min(90, round(index * 0.45)))
        started = perf_counter_ns()
    ...
    assert roles_changed(roles_before_move, capture_roles(ids))
    ...
    for index in range(1, EVENTS + 1):
        final_resize = QPoint(resize_start.x() + min(140, round(index * 0.7)), resize_start.y() + min(84, round(index * 0.42)))
        started = perf_counter_ns()
    ...
    assert roles_changed(roles_before_resize, capture_roles(ids))
...
def main() -> int:
    ...
    output = {
        ...
        "all_presets_save_reopen": all(bool(item["save_reopen_preserved"]) for item in results),
        "pass": True,
    }
