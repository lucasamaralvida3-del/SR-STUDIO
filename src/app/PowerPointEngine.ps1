...
try {
    ...
    foreach ($job in $jobs) {
        ...
        try {
            ...
            if ([int]$job.tipo -eq 1) {
                $model = if ($hasLimit) { $Model1Limit } else { $Model1 }
            } elseif ([int]$job.tipo -eq 2) {
                $model = if ($hasLimit) { $Model2Limit } else { $Model2 }
            } elseif ([int]$job.tipo -eq 3) {
                $model = if ($hasLimit) { $ClubModelLimit } else { $ClubModel }
            } else {
                $model = $SaleModel
            }
            ...
            Write-Output ("STAGE|{0}|PREENCHENDO_CAMPOS" -f $idx)
            ...
            if ([int]$job.tipo -eq 1) {
                ...
                if ($hasLimit) {
                    ...
                    $limitText = Format-Limit ([string]$job.limite)
                    Set-TextPreserveStyle $limit $limitText
                }
            }
            elseif ([int]$job.tipo -eq 2) {
                $campaign = Get-ShapeByName $slide "SR_CAMPANHA"
            ...
            $safeCampaign = Safe-FileName ([string]$job.campanha)
            $safeProduct  = Safe-FileName ([string]$job.produto)
            ...
            if (-not (Test-Path -LiteralPath $pdf)) {
                throw "O PowerPoint não criou o PDF: $pdf"
            }

            $outFiles += $pdf
            $modelType = if ($hasLimit) { "COM LIMITE" } else { "SEM LIMITE" }
            Write-Output ("OK|{0}|{1}|{2}" -f $idx, $pdf, $modelType)
        }
        finally {
            # Não força FinalReleaseComObject/GC aqui. Em algumas versões do Office
            # isso pode bloquear depois que o PDF já foi salvo.
            if ($null -ne $pres) {
    ...
    $outFiles | Set-Content -LiteralPath $manifest -Encoding UTF8
    Write-Output ("BATCH_DONE|{0}" -f $outFiles.Count)
}
finally {
    if ($null -ne $ppt) {
        try { $ppt.Quit() } catch {}
