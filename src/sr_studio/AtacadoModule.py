...
from SRStudio21 import dated_output_dir, smart_pdf_name, unique_path, record_product_jobs, record_reprint, pdf_with_copies, apply_learned_correction
...
def build_posters(items):
        ...
        if rule and rule.get("nome_cartaz"):
            ...
            if q=="FRAGRÂNCIAS" and core.startswith("SABONETE ") and measure:
                ...
                if measure and norm(measure) not in norm(core):
                    core=f"{core} {measure}"
                name=f"{core} {q}"
            name=apply_learned_correction(" ".join(name.split()))
        first=members[0]
        p={
    ...
    for member_list in singles:
        for it in member_list:
            posters.append({
                "cartaz_chave":"P:"+it["codigo"],"nome":apply_learned_correction(it["descricao"]),
