...
def font_info(sp):
    ...
    if rpr is None:rpr=sp.find('.//a:endParaRPr',NS)
    out={'font':'Segoe UI','fontSize':28.0,'bold':False,'italic':False,'color':'#172033','align':'left','vAlign':'center'}
    if rpr is not None:
    ...
    ppr=sp.find('.//a:p/a:pPr',NS)
    algn=(ppr.get('algn') if ppr is not None else '') or ''
    out['align']={'ctr':'center','r':'right','just':'center','dist':'center'}.get(algn,'left')
    body=sp.find('.//a:bodyPr',NS)
    anchor=(body.get('anchor') if body is not None else '') or ''
    out['vAlign']={'t':'top','ctr':'center','b':'bottom'}.get(anchor,'center')
    return out
    ...
    for i,a in enumerate(sorted(anchors,key=lambda z:(z['y'],z['x'])),1):
        ...
        if 'IMAGEM' not in fields:
            ...
            if c:
                ...
                if dist(a,b)<=maxd*1.85:fields['IMAGEM']=b;used_areas.add(b['id'])
        ...
        slots.append({'id':f'slot_{i}','index':i,'x':x,'y':y,'w':r-x,'h':bt-y,'fields':{k:{'x':e['x'],'y':e['y'],'w':e['w'],'h':e['h'],'style':e.get('style',{}),'sourceId':e['id'],'autoDetected':bool(e.get('imageCandidate')),'fit':(e.get('style') or {}).get('fit','contain')} for k,e in fields.items()}})
    for e in elements:e['placeholder']=bool(e.get('role') and e.get('role')!='IMAGEM_AREA' and e['id'] in used)
    return slots
