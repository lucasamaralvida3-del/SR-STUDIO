...
def launch_qt_quick_editor(document: GraphicsDocument | None = None) -> int:
    ...
    class SceneBridge(QObject):
        ...
        def selectNode(self, node_id: str) -> None:
            session.select(node_id); self.sceneChanged.emit()
        @Slot(float, float)
        def moveSelection(self, dx: float, dy: float) -> None:
            session.move_selected(dx, dy); self.sceneChanged.emit()
        @Slot()
        def undo(self) -> None:
            if session.undo(): self.sceneChanged.emit()
        @Slot()
        def redo(self) -> None:
            if session.redo(): self.sceneChanged.emit()
        @Slot(str, str)
        def editText(self, node_id: str, text: str) -> None:
            session.set_text(node_id, text); self.sceneChanged.emit()
        @Slot(str)
        def dispatch(self, payload: str) -> None:
            try:
                command: dict[str, Any] = json.loads(payload); name = str(command.get("name") or "")
                if name == "lock": session.lock_selected(bool(command.get("value", True)))
                elif name == "hide": session.hide_selected(bool(command.get("value", True)))
                elif name == "layer": session.layer_selected(str(command.get("mode") or "front"))
                elif name == "rotate": session.rotate_selected(float(command.get("angle") or 0))
                elif name == "opacity": session.set_opacity(float(command.get("value") or 1))
                elif name == "duplicate": session.duplicate_selected()
                elif name == "delete": session.delete_selected()
                elif name == "align": session.align_selected(str(command.get("mode") or "left"))
                elif name == "distribute": session.distribute_selected(str(command.get("axis") or "horizontal"))
                else: raise ValueError(f"Comando desconhecido: {name}")
                self._status = f"Comando executado: {name}"
            except Exception as exc:
                self._status = f"Erro: {exc}"
            self.statusChanged.emit(); self.sceneChanged.emit()
