        ...
        try:
            ...
            if name == "delete_page":
                ...
                return CommandResult(
                    True,
                    changed,
                    "Página excluída." if changed else "A última página do projeto não pode ser excluída.",
                )
            ...
            if name == "reorder_page":
                ...
                return CommandResult(
                    True,
                    changed,
                    "Página reordenada." if changed else "Página já está nessa posição.",
                    {"page_id": page_id, "index": final_index, "page_ids": [page.id for page in pages]},
                )
            ...
            if name == "replace_image":
                ...
                changed = self.professional.replace_image(
                    node_id,
                    source,
                    reset_framing=bool(command.get("reset_framing", False)),
                )
                return CommandResult(
                    bool(changed),
                    bool(changed),
                    "Imagem substituída preservando o enquadramento." if changed else "Imagem não pôde ser substituída.",
                    {"node_id": node_id},
                )
            ...
            if name == "edit_price_block":
                ...
                changed = self.professional.edit_price_block(
                    block_id,
                    command.get("price"),
                    unit=command.get("unit") if "unit" in command else None,
                    currency=str(command.get("currency") or "R$"),
                )
            ...
            if name == "edit_product_card":
                ...
                kwargs = {
                    key: command[key]
                    for key in ("name", "price", "unit", "image_source", "limit", "app_price")
                    if key in command
                }
            ...
            if name == "inspect_usability":
                report = self.professional.inspect_usability(
                    require_multi_product_page=bool(command.get("require_multi_product_page", False))
                )
                return CommandResult(
                    True,
                    False,
                    "Estrutura pronta para edição." if report.professional_usable else "Há bloqueios estruturais no projeto.",
                    report.to_dict(),
                )
