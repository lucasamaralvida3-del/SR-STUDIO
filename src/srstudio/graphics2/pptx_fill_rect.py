...
def recover_pptx_fill_rects(source: str | Path, document: GraphicsDocument) -> PptxFillRectRecoveryReport:
    ...
    for contract in contracts:
        ...
        # Leia novamente da própria SR Scene para garantir que a serialização
        # efetiva do node representa o contrato, não apenas que tentamos aplicar.
...
def _shape_identity(node: ET.Element, kind: str) -> tuple[str, str]:
    path = (
        ...
        if kind == "sp"
        else f"./{{{P_NS}}}nvPicPr/{{{P_NS}}}cNvPr"
    )
