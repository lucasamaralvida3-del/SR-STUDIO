...
def _recover_complete_price_blocks(page: GraphicsPage, semantic_module: Any, existing: list[Any]) -> list[Any]:
    ...
    output: list[Any] = []
    # Ordem visual torna a recuperação determinística em grades e em cartões
    # com mais de um preço (varejo/clube/app).
...
def _nearest_cluster_token(
    ...
    *,
    ...
    """Seleciona moeda/unidade por proximidade de caixas, não por lado fixo.

    WordArt de preço costuma possuir uma caixa muito larga. Distância entre
    centros penalizaria ``R$``/``CADA`` mesmo quando as bordas estão separadas
    por poucos pixels. A distância entre retângulos representa melhor a
    composição visual e também funciona quando a unidade está à direita.
    """
    ...
    for node in candidates:
        ...
        # Moeda normalmente acompanha a metade superior/central do preço;
        # unidade acompanha a metade central/inferior. A tolerância permanece
        # larga o suficiente para WordArt sem depender de offsets do template.
