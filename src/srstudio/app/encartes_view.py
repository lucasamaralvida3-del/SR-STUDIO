...
class InteractiveFlyerCanvas(tk.Canvas):
    ...
    def redraw(self) -> None:
        ...
        self.create_rectangle(bounds.x - 6, bounds.y - 6, bounds.right + 6, bounds.bottom + 6, fill="#D5DFEC", outline="")
        self.create_rectangle(bounds.x, bounds.y, bounds.right, bounds.bottom, fill=self.controller.page.background, outline="#C8D2E1")
    ...
    def _draw_element(self, transform: ViewportTransform, element: dict, key: str) -> None:
        x, y = transform.to_screen(float(element.get("x", 0)), float(element.get("y", 0)))
        width = float(element.get("width", 0)) * transform.scale
        height = float(element.get("height", 0)) * transform.scale
        kind = element.get("type")
        if kind == "rect":
            self.create_rectangle(x, y, x + width, y + height, fill=element.get("fill", "#FFFFFF"), outline=element.get("outline", ""))
        elif kind == "text":
            size = max(7, int(float(element.get("font_size", 20)) * transform.scale))
            self.create_text(
                x,
                y,
                ...
                font=(FONT["family"], size, "bold" if element.get("bold") else "normal"),
            )
        elif kind == "image":
            photo = self._thumbnail(str(element.get("path", "")), max(1, int(width)), max(1, int(height)), key)
            if photo is not None:
                self.create_image(x + width / 2, y + height / 2, image=photo)

    def _draw_card(self, transform: ViewportTransform, card: ProductCard) -> None:
        ...
        self.create_rectangle(
            ...
            rect.right,
            rect.bottom,
            fill=vm.style.background,
            outline=COLORS.primary if selected else vm.style.border,
            width=2 if selected else 1,
        )
        image_region = vm.style.image_region
        image_x = rect.x + image_region.x * rect.width
        image_y = rect.y + image_region.y * rect.height
        image_w = max(4, image_region.width * rect.width)
        image_h = max(4, image_region.height * rect.height)
        photo = self._thumbnail(vm.image_path, int(image_w), int(image_h), f"card-{card.id}")
        if photo is not None:
            self.create_image(image_x + image_w / 2, image_y + image_h / 2, image=photo)
        else:
            self.create_rectangle(image_x, image_y, image_x + image_w, image_y + image_h, fill="#F4F7FB", outline="")
            self.create_text(
                image_x + image_w / 2,
                image_y + image_h / 2,
                text="SEM IMAGEM",
                fill="#94A3B8",
                font=(FONT["family"], max(6, int(9 * transform.scale)), "bold"),
            )
        name_region = vm.style.name_region
        self.create_text(
            rect.x + name_region.x * rect.width,
            rect.y + name_region.y * rect.height,
            text=vm.name,
            anchor="nw",
            width=max(20, name_region.width * rect.width),
            fill=vm.style.text_color,
            font=(FONT["family"], max(7, int(14 * transform.scale)), "bold"),
        )
        price_region = vm.style.price_region
        price_x = rect.x + price_region.x * rect.width
        price_y = rect.y + price_region.y * rect.height
        price_scale = max(0.4, min(float(card.overrides.get("price_scale", 1.0)), 3.0))
        self.create_text(
            price_x,
            price_y + 5 * transform.scale,
            text=vm.currency,
            anchor="nw",
            fill=vm.style.price_color,
            font=(FONT["family"], max(6, int(10 * transform.scale * price_scale)), "bold"),
        )
        self.create_text(
            price_x + 18 * transform.scale,
            price_y,
            text=vm.integer,
            anchor="nw",
            fill=vm.style.price_color,
            font=(FONT["family"], max(12, int(36 * transform.scale * price_scale)), "bold"),
        )
        self.create_text(
            price_x + max(38, len(vm.integer) * 24) * transform.scale,
            price_y + 5 * transform.scale,
            text=f",{vm.decimal}",
            anchor="nw",
            fill=vm.style.price_color,
            font=(FONT["family"], max(8, int(19 * transform.scale * price_scale)), "bold"),
        )
        ...
        if vm.limit and bool(card.overrides.get("show_limit", True)):
            self.create_text(
                rect.x + 8,
                rect.bottom - 7,
                text=f"LIMITE {vm.limit} POR CPF",
                anchor="sw",
                fill="#64748B",
                font=(FONT["family"], max(5, int(8 * transform.scale))),
            )
        if card.highlighted:
            self.create_text(
                rect.right - 7,
                ...
                anchor="ne",
                fill="#F2B705",
                font=(FONT["family"], max(9, int(17 * transform.scale)), "bold"),
            )
        if card.locked:
            self.create_text(rect.x + 7, rect.y + 7, text="●", anchor="nw", fill="#64748B", font=(FONT["family"], 7))

    def _draw_selection(self, transform: ViewportTransform) -> None:
        for card in self.controller.scene.selected():
            rect = transform.rect_to_screen(Rect(card.x, card.y, card.width, card.height))
            self.create_rectangle(rect.x, rect.y, rect.right, rect.bottom, outline="#2563EB", width=2, dash=(4, 2))
            if not card.locked:
                handle = resize_handle(rect, self.HANDLE)
    ...
    def _mouse_down(self, event: tk.Event) -> None:
        ...
        if len(selected) == 1 and not selected[0].locked:
            card = selected[0]
            rect = transform.rect_to_screen(Rect(card.x, card.y, card.width, card.height))
            if contains(resize_handle(rect, self.HANDLE + 4), event.x, event.y):
        ...
        self._drag_last = transform.to_page(event.x, event.y)
        self._notify_selection()
        self.redraw()
    ...
    def _mouse_drag(self, event: tk.Event) -> None:
        ...
        if self._resize_card_id:
            ...
            if card is not None:
                self.controller.scene.resize(card.id, max(32, page_x - card.x), max(32, page_y - card.y))
                self.redraw()
                self._changed()
            return
    ...
    def _show_guides(self, transform: ViewportTransform, guides) -> None:
        self._clear_guides()
        bounds = transform.page_bounds()
        for guide in guides:
            if guide.axis == "x":
                x, _ = transform.to_screen(guide.value, 0)
                self._guide_items.append(self.create_line(x, bounds.y, x, bounds.bottom, fill="#E11D48", dash=(4, 4)))
            else:
                _, y = transform.to_screen(0, guide.value)
...
class EncartesStudioView(tk.Frame):
    ...
    def _build(self) -> None:
        ...
        actions = (
            ("↶", self._undo),
            ("↷", self._redo),
            ("Layout automático", self._auto_layout),
            ("★ Destaque", self._toggle_highlight),
            ("＋ Página", self._add_page),
            ("Duplicar página", self._duplicate_page),
        )
        ...
        body.add(self.left, minsize=220)
        body.add(self.center, minsize=520)
        body.add(self.right, minsize=220)
        self._build_library()
    ...
    def _refresh_products(self) -> None:
        ...
        for product in products[:40]:
            ...
            tk.Label(
                info,
                text=f"{parts.currency} {parts.integer},{parts.cents} /{product.unit}",
                anchor="w",
                bg="#F8FAFD",
                fg=COLORS.primary,
                font=(FONT["family"], 8, "bold"),
    ...
    def _selection_changed(self, cards: list[ProductCard]) -> None:
        ...
        if not cards:
            tk.Label(
                self.property_body,
                text="Selecione um produto no encarte.",
                justify="left",
                wraplength=220,
                bg=COLORS.surface,
                fg=COLORS.text_muted,
        ...
        for label, key, value in fields:
            ...
            entry = tk.Entry(self.property_body, relief="flat", bg="#F5F7FB", fg=COLORS.text)
        ...
        ttk.Button(
            ...
            command=lambda: self._apply_properties(card, product),
        ).pack(fill="x", pady=(12, 5))
        ttk.Button(
            self.property_body,
            text="Bloquear / desbloquear",
            command=lambda: self._toggle_lock(card),
        ).pack(fill="x", pady=3)
    ...
    def _refresh_pages(self) -> None:
        ...
        for index, page in enumerate(self.project.pages):
            ...
            tk.Button(
                ...
                bg="#E8F0FE" if active else COLORS.surface,
                fg=COLORS.primary if active else COLORS.text,
                bd=0,
                padx=14,
                pady=5,
        ...
        tk.Label(
            self.pagebar,
            text=f"Zoom {round(self.canvas.zoom * 100)}%",
            bg=COLORS.surface,
            fg=COLORS.text_muted,
    ...
    def _switch_page(self, index: int) -> None:
        self.controller = EditorController(self.project, self.project.pages[index])
    ...
    def _add_page(self) -> None:
        page = self.pages.add_page(copy_master_from=self.controller.page)
        self._changed()
        self._switch_page(self.project.pages.index(page))

    def _duplicate_page(self) -> None:
        index = self.project.pages.index(self.controller.page)
        page = self.pages.duplicate(index)
        self._changed()
        self._switch_page(self.project.pages.index(page))

    def _add_product(self, product: Product) -> None:
        self.controller.add_product(product, x=40, y=40)
        self._changed()
        self.canvas.redraw()
    ...
    def _auto_layout(self) -> None:
        highlighted = sum(card.highlighted for card in self.controller.page.cards)
        self.controller.apply_auto_layout(highlighted=highlighted)
        self._changed()
        self.canvas.redraw()
        self._refresh_quality()
    ...
    def _zoom(self, delta: float) -> None:
        self.canvas.zoom = min(2.5, max(0.35, self.canvas.zoom + delta))
        self.canvas.redraw()
        self._refresh_pages()
    ...
    def _refresh_quality(self) -> None:
        try:
            ...
            color = COLORS.success if report.total >= 90 else "#E69200"
            self.quality_label.configure(text=f"Qualidade {report.total}/100", fg=color)
        except Exception:
