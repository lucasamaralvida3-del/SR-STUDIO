...
def font_info(sp):
    ...
    out['bold']=str(rpr.get('b') or '').lower() in {'1','true'}
    out['italic']=str(rpr.get('i') or '').lower() in {'1','true'}
...
def _image_score(anchor,candidate):
    """Prioriza caixa de foto próxima do preço e, em empate, acima/à esquerda dele."""
    d=dist(anchor,candidate)
    acx=anchor['x']+anchor['w']/2;acy=anchor['y']+anchor['h']/2
    ccx=candidate['x']+candidate['w']/2;ccy=candidate['y']+candidate['h']/2
    penalty=0.0
    if ccy>acy+anchor['h']*1.2: penalty+=80
    if abs(ccx-acx)>max(candidate['w'],anchor['w'])*3.2: penalty+=60
    return d+penalty
...
def make_slots(elements,w,h):
    ...
    image_areas=[e for e in elements if e.get('imageCandidate')]
    anchors=[e for e in text if e.get('role')=='PRECO_REAIS']
    if not anchors:
    ...
    slots=[];used=set();used_areas=set();maxd=max(160,min(w,h)*.30)
    for i,a in enumerate(sorted(anchors,key=lambda z:(z['y'],z['x'])),1):
        ...
        if 'NOME' not in fields:
            ...
            if c:
                b=min(c,key=lambda e:dist(a,e))
                if dist(a,b)<=maxd*1.6:
                    b['role']='NOME';fields['NOME']=b;used.add(b['id'])

        # 1) placeholder de imagem explícito; 2) caixa branca/shape detectada automaticamente.
        c=[e for e in imgs if e['id'] not in used and e.get('role')=='IMAGEM']
        if c:
            b=min(c,key=lambda e:dist(a,e))
            if dist(a,b)<=maxd*1.8:fields['IMAGEM']=b;used.add(b['id'])
        if 'IMAGEM' not in fields:
            c=[e for e in image_areas if e['id'] not in used_areas]
            if c:
                b=min(c,key=lambda e:_image_score(a,e))
                if dist(a,b)<=maxd*1.85:
                    fields['IMAGEM']=b;used_areas.add(b['id'])
        ...
        if 'PRECO_CENTAVOS' not in fields:
            ...
            if c:
                b=min(c,key=lambda e:dist(a,e))
                if dist(a,b)<=maxd:
                    b['role']='PRECO_CENTAVOS';fields['PRECO_CENTAVOS']=b;used.add(b['id'])

        vals=list(fields.values())
        x=min(e['x'] for e in vals);y=min(e['y'] for e in vals)
        r=max(e['x']+e['w'] for e in vals);bt=max(e['y']+e['h'] for e in vals)
        slots.append({
            'id':f'slot_{i}','index':i,'x':x,'y':y,'w':r-x,'h':bt-y,
            'fields':{k:{'x':e['x'],'y':e['y'],'w':e['w'],'h':e['h'],'style':e.get('style',{}),'sourceId':e['id'],'autoDetected':bool(e.get('imageCandidate'))} for k,e in fields.items()}
        })

    # Somente campos dinâmicos reais são ocultados do render do PowerPoint.
    # A caixa branca detectada automaticamente permanece no fundo do design.
    for e in elements:
        e['placeholder']=bool(e.get('role') and e.get('role')!='IMAGEM_AREA' and e['id'] in used)
    return slots
