...
def _text_elements(root,index,sx,sy):
    ...
    for sp in root.findall('.//p:sp',NS):
        ...
        out.append({'id':f's{index}_t{seq}','type':'text','name':name,'text':text,'x':x*sx,'y':y*sy,'w':w*sx,'h':h*sy,'style':style,'role':role(text,name,style.get('fontSize',0))})
    return out
...
def _images(z,root,rels,index,sx,sy,asset_dir):
    ...
    for pic in root.findall('.//p:pic',NS):
        ...
        out.append({'id':f's{index}_i{seq}','type':'image','name':name,'url':url,'x':x*sx,'y':y*sy,'w':w*sx,'h':h*sy,'style':{'fit':'contain'},'role':r})
    return out
...
def parse_pptx(data:bytes,source_name='modelo.pptx'):
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as z:
        ...
        for index,slide_name in enumerate(_slides(z.namelist()),1):
            ...
            pages.append({'id':f'pptx_{index}','name':f'Página {index}','width':page_w,'height':page_h,'templateElements':elements,'templateSlots':make_slots(elements,page_w,page_h),'elements':[]})
    return {'ok':True,'source':source_name,'session':session,'pages':pages,'pageCount':len(pages),'slotCount':sum(len(p['templateSlots']) for p in pages)}
