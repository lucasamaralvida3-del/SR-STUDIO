...
def test_manual_save_thread_marks_recent_project_but_main_thread_save_does_not(tmp_path, monkeypatch):
    root = tmp_path / "state"
    monkeypatch.setattr(editor_entrypoint, "default_autosave_root", lambda: root)
    monkeypatch.setattr(editor_entrypoint.package_module, "save_package", real_save_package)
    editor_entrypoint.install_manual_save_recent_project_hook()

    document = GraphicsDocument(name="Entry point")
    main_target = tmp_path / "main-thread.srscene"
    editor_entrypoint.package_module.save_package(document, main_target)
    assert EditorRecentProject(root).current() is None

    manual_target = tmp_path / "manual.srscene"
    failure: list[BaseException] = []

    def worker() -> None:
        try:
            editor_entrypoint.package_module.save_package(document, manual_target)
    ...
    assert recent.path == manual_target.resolve()
